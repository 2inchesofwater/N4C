...
/**
 * Event Booking component Coupons Model
 *
 * @package		Joomla
 * @subpackage	Event Booking
 * @since 1.5
 */
class EventBookingModelCoupons extends JModel
{
/**
	 * Coupons data array
	 *
	...
	/**
	 * Method to get the total number of coupons
	 *
	 * @access public
	 * @return integer
	 */
	function getTotal()
	{
		...
		if (empty($this->_total))
		{
			...
			$sql  = 'SELECT COUNT(*) FROM #__eb_coupons AS a '.$where;
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT a.* FROM #__eb_coupons AS a '			
			. $where			
			. $orderby
		;		
		return $query;
	...
	function _buildContentWhere()
	{
		...
		$where = array();			
		if ($search) {
			$where[] = 'LOWER(a.code) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}						
